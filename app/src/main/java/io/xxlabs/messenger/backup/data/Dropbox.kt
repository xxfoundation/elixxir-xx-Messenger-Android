...
import io.xxlabs.messenger.backup.auth.DropboxAuthHandler
...
class Dropbox private constructor(
    ...
    private suspend fun accessDropboxAppData() = withContext(Dispatchers.IO) {
        dbxInstance?.run {
            try {
                ...
                _lastBackup.postValue(dbxBackupData)
            } catch (e: Exception) {
    ...
    companion object {
        ...
        fun getInstance(backupService: BackupService, preferences: PreferencesRepository): Dropbox =
